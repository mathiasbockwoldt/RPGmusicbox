class Sound():
	...
	def __init__(self, filename, name, volume=1, cooldown=10, occurrence=0.01, loop=False):
		...
		:param loop: Boolean whether the sound shall be played indefinitely or not.
		    `occurrence` is disregarded when loop is True.
		'''
...
class GlobalEffect():
	...
	def __init__(self, filename, key, name, volume=1, interrupting=True):
		...
		:param name: String with the name
		:param volume: Float with the relative volume (already adjusted by the theme volume)
		:param interrupting: Boolean that indicates, whether the global effect
		    should interrupt playing music and sounds, or not.
		'''
	...
	def __str__(self):
		...
		if self.interrupting:
			...
			inter = ''
		return '{}) {}: {} (vol: {}{})'.format(self.key, self.name, self.filename, self.volume, inter)
...
class Theme():
	...
	def __init__(self, kid, name, colors):
		'''
		Initiates the theme.
	...
	def add_occurrences(self, occurrences):
		'''
		Adds a list of occurrences to the theme. The total number of occurrences
		after the addition must be the same as the total number of songs in the
		theme. So add the songs first.

		:param occurrences: List of occurrences of the songs
		:raises KeyError: When the total number of occurrences does not fit the
		    total number of songs in the theme
		'''
		...
		if len(self.occurrences) != len(self.sounds):
			raise KeyError(
				'The number of sounds is not equal to the number of occurrences in {}!'.
				format(self.name)
			)
